-- Fix submit_quadratic_vote to only apply delegation multiplier when enabled
DROP FUNCTION IF EXISTS public.submit_quadratic_vote(uuid, jsonb);

CREATE OR REPLACE FUNCTION public.submit_quadratic_vote(p_election_id uuid, p_allocations jsonb)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  liquid_delegation_enabled boolean;
BEGIN
  ...
  -- Check if liquid delegation is enabled for this election
  SELECT COALESCE((voting_logic_config->>'allowLiquidDelegation')::boolean, false)
  INTO liquid_delegation_enabled
  FROM elections
  ...
  IF prev_vote_id IS NOT NULL THEN
    ...
    IF prev_related IS NOT NULL AND jsonb_typeof(prev_related) = 'array' AND jsonb_array_length(prev_related) > 0 THEN
      ...
      SELECT ARRAY(SELECT (jsonb_array_elements_text(prev_related))::uuid) INTO related_vote_ids;
      
      DELETE FROM public.anonymous_votes
      WHERE id = ANY(related_vote_ids);
    ELSE
  ...
  -- Only get delegator count if liquid delegation is enabled
  IF liquid_delegation_enabled THEN
    SELECT result.delegator_count INTO v_delegator_count
    FROM public.get_valid_delegators_for_election(auth.uid(), p_election_id) AS result;
  END IF;
  ...
  LOOP
    ...
    IF option_text IS NOT NULL AND base_votes_int > 0 THEN
      ...
      VALUES (
        ...
        p_election_id,
        option_text,
        (base_votes_int * (1 + COALESCE(v_delegator_count, 0)))::numeric,
        jsonb_build_object(
          ...
          'delegations_count', COALESCE(v_delegator_count, 0),
          'related_votes', '[]'::jsonb
        )
      );
    END IF;
